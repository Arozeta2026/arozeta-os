import Link from "next/link";
import income from "@/data/income.json";
import ExecutiveCard from "@/components/ui/ExecutiveCard";

export default function IncomeWidget() {

  const items = income as any[];

  const total = items.reduce(
    (sum, item) => sum + Number(item.Importe || 0),
    0
  );

  const money = (value: number) =>
    new Intl.NumberFormat("es-ES", {
      style: "currency",
      currency: "EUR",
      maximumFractionDigits: 0,
    }).format(value);

  const topIncome = [...items]
    .sort(
      (a, b) =>
        Number(b.Importe || 0) -
        Number(a.Importe || 0)
    )
    .slice(0, 5);

  return (
    <ExecutiveCard
      title="Income"
      subtitle={`${items.length} ingresos registrados`}
    >

      <div className="space-y-6">

        <div className="rounded-2xl bg-emerald-900/20 border border-emerald-700/30 p-5">

          <p className="text-sm text-slate-400">
            Total mensual
          </p>

          <p className="mt-2 text-4xl font-bold text-emerald-400">
            {money(total)}
          </p>

        </div>

        <div className="space-y-3">

          {topIncome.map((item, index) => (

            <div
              key={index}
              className="flex justify-between rounded-xl bg-slate-800 px-4 py-3"
            >

              <div>

                <p className="font-medium text-white">
                  {item.Concepto}
                </p>

                <p className="text-sm text-slate-500">
                  {item.Empresa}
                </p>

              </div>

              <span className="font-semibold text-emerald-400">
                {money(Number(item.Importe))}
              </span>

            </div>

          ))}

        </div>

        <Link
          href="/income"
          className="block rounded-xl bg-emerald-600 py-3 text-center font-semibold text-white hover:bg-emerald-500"
        >
          Ver todos los ingresos →
        </Link>

      </div>

    </ExecutiveCard>
  );
}